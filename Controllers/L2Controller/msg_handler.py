...
def flow_mod_handler(cc_agent, version, msg_type, msg_len, xid, data):
    ...
    msg.msg_len = msg_len
    msg.xid = xid
    msg.in_dpid, msg.out_dpid, msg.out_port, msg.dst_mac, msg.wildcards = struct.unpack_from(
...
def switch_controller_map_update(cc_agent, version, msg_type, msg_len, xid, data):
    msg = msg_proto_parser.TFLCPSwitchControllerMapUpdate(cc_agent)
    msg.version = version
    msg.msg_type = msg_type
    msg.msg_len = msg_len
    msg.xid = xid
    ev = tflc_event.EventTFLCPSwitchControllerMapUpdate(msg)
    EVENT_DISPATCHER.dispatch_event(ev)
