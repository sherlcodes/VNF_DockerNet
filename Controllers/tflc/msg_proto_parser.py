...
class TFLCPPacketIn(MsgUpBase):
    def __init__(self, local_ctrl, in_dpid = None, src_ip = None, dst_ip = None, 
        src_port = None, dst_port = None , src_mac = None, dst_mac = None):
        super(TFLCPPacketIn, self).__init__(local_ctrl)
        self.in_dpid = in_dpid
        self.src_ip = None
        self.dst_ip = None
        if src_ip:
            self.src_ip = socket.inet_ntoa(struct.pack('!L', src_ip))
        if dst_ip:
            self.dst_ip = socket.inet_ntoa(struct.pack('!L', dst_ip))
...
class TFLCPFlowMod(MsgDownBase):
    def __init__(self, local_ctrl, in_dpid = None, out_dpid = None,out_port=None, dst_mac = None, wildcards = None):
    ...
    def _serialize_body(self):
        msg_pack_into(msg_proto.TFLCP_FLOW_MOD_PACK_STR, self.buf, msg_proto.TFLCP_HEADER_SIZE,
                      self.in_dpid, self.out_dpid, self.out_port, self.dst_mac, self.wildcards)
...
class TFLCPSwitchControllerMapUpdate(MsgDownBase):
    def __init__(self, local_ctrl):
        super(TFLCPSwitchControllerMapUpdate, self).__init__(local_ctrl)
        self.msg_type = msg_proto.TFLCT_SWITCH_MAP_UPDATE
...
class TFLCPGraphInfo(MsgUpBase):
    def __init__(self, local_ctrl):
        super(TFLCPGraphInfo, self).__init__(local_ctrl)
