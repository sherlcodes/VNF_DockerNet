...
def packet_in_handler(local_ctrl, version, msg_type, msg_len, xid, data):
    ...
    msg.msg_len = msg_len
    msg.xid = xid
    msg.in_dpid, msg.src_ip, msg.dst_ip, msg.src_port, msg.dst_port, msg.src_mac, msg.dst_mac,  = struct.unpack_from(
...
def graph_info_handler(local_ctrl, version, msg_type, msg_len, xid, data):
    msg = msg_proto_parser.TFLCPGraphInfo(local_ctrl)
    msg.version = version
    msg.msg_type = msg_type
    msg.msg_len = msg_len
    msg.xid = xid
    len = struct.unpack_from("I", data, msg_proto.TFLCP_HEADER_SIZE)
    len, msg.json_window_switches = struct.unpack_from("I%ds" %len, data, msg_proto.TFLCP_HEADER_SIZE)
    ev = tflc_event.EventTFLCPGraphInfo(msg)
    EVENT_DISPATCHER.dispatch_event(ev)
...
def switch_controller_map_update():
    pass
...
msg_handler = {msg_proto.TFLCT_HELLO_UP: hello_up_handler,  # up
               ...
               msg_proto.TFLCT_GRAPH_INFO: graph_info_handler, #up
               msg_proto.TFLCT_SWITCH_MAP_UPDATE: switch_controller_map_update #down
               }
