...
class CentralCtrlAgent(object):
    ...
    def _recv_loop(self):
        ...
        while self.is_live:
            ...
            # append in buf
            buf += ret
            # if there are multiple messages it will parse all
            while len(buf) >= required_len:
                (version, msg_type, msg_len, xid) = msg_proto_parser.header(buf)
                # after reading header we know how much length we are expecting buf should not
                # have less length.
    ...
    def serve(self):
        while True:
            print "Trying to connect %s:%s"%(cfg.CC_HOST, cfg.CC_PORT)
            try:
