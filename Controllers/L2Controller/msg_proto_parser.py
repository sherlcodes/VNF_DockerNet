...
# Graphinfo is actually the info about --> up
class TFLCPGraphInfo(MsgUpBase):
    def __init__(self, cc_agent, window_switches = None):
        super(TFLCPGraphInfo, self).__init__(cc_agent)
        self.msg_type = msg_proto.TFLCT_GRAPH_INFO
        self.json_window_switches = json.dumps(window_switches)

    def _serialize_body(self):
        self.json_window_switches = str(self.json_window_switches)
        msg_pack_into("I%ds" % len(self.json_window_switches),self.buf, msg_proto.TFLCP_HEADER_SIZE,
                        len(self.json_window_switches),self.json_window_switches)
...
class TFLCPPacketIn(MsgUpBase):
    def __init__(self, cc_agent, in_dpid = None, src_mac = None, dst_mac = None,src_ip = None, dst_ip = None, tp_src = None, tp_dst = None):
        super(TFLCPPacketIn, self).__init__(cc_agent)
        print in_dpid, src_mac, dst_mac,src_ip, dst_ip , tp_src , tp_dst
        ...
        if src_ip == None:
            ...
            packedIP = socket.inet_aton(src_ip)
            self.src_ip = struct.unpack("!L", packedIP)[0]
        if dst_ip == None:
            self.dst_ip = 0
        else:
            packedIP = socket.inet_aton(dst_ip)
            self.dst_ip = struct.unpack("!L", packedIP)[0]          
        self.tp_src = tp_src            # 16 bits
        self.tp_dst = tp_dst            # 16 bits

        # print type(self.in_dpid), type(self.src_mac), type(self.dst_mac), type(self.src_ip), type(self.dst_ip), type(self.tp_src), type(self.tp_dst)

    def _serialize_body(self):
        msg_pack_into(msg_proto.TFLCP_PACKET_IN_PACK_STR, self.buf, msg_proto.TFLCP_HEADER_SIZE, 
            self.in_dpid, self.src_ip, self.dst_ip, self.tp_src, self.tp_dst , self.src_mac, 
            self.dst_mac)
...
class TFLCPFlowMod(MsgDownBase):
    def __init__(self, cc_agent, in_dpid = None, out_dpid = None, out_port = None, dst_mac = None, wildcards = None):
...
# Switch controller mapping update
class TFLCPSwitchControllerMapUpdate(MsgDownBase):
    def __init__(self, cc_agent):
        super(TFLCPSwitchControllerMapUpdate, self).__init__(cc_agent)
